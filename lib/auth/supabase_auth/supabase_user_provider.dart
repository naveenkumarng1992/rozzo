...
class MedmaxSupabaseUser extends BaseAuthUser {
  MedmaxSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> medmaxSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = MedmaxSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
